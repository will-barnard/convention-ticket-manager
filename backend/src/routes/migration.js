...
router.post('/receive', async (req, res) => {
  try {
    const { secret, tickets, ticket_scans, ticket_supplies } = req.body;
    ...
    console.log(`Receiving migration with ${tickets?.length || 0} tickets, ${ticket_scans?.length || 0} scans, ${ticket_supplies?.length || 0} supplies`);
    ...
    try {
      ...
      if (tickets && tickets.length > 0) {
        for (const ticket of tickets) {
          await db.query(`
            INSERT INTO tickets (ticket_type, name, teacher_name, email, uuid, is_used, used_at, created_at)
      ...
      // Insert ticket supplies
      if (ticket_supplies && ticket_supplies.length > 0) {
        for (const supply of ticket_supplies) {
          // Get the ticket ID from UUID
          const ticketResult = await db.query('SELECT id FROM tickets WHERE uuid = $1', [supply.ticket_uuid]);
          
          if (ticketResult.rows.length > 0) {
            const ticketId = ticketResult.rows[0].id;
            
            // Delete existing supplies for this ticket
            await db.query('DELETE FROM ticket_supplies WHERE ticket_id = $1', [ticketId]);
            
            // Insert new supply
            await db.query(
              'INSERT INTO ticket_supplies (ticket_id, supply_name, quantity, created_at) VALUES ($1, $2, $3, $4)',
              [ticketId, supply.supply_name, supply.quantity, supply.created_at]
            );
            suppliesInserted++;
          }
        }
      ...
      console.log(`Migration completed: ${ticketsUpserted} tickets upserted, ${scansInserted} scans inserted, ${suppliesInserted} supplies inserted`);
...
router.post('/send', superAdminMiddleware, async (req, res) => {
  try {
    ...
    // Fetch all ticket supplies with ticket UUIDs
    const suppliesResult = await db.query(`
      SELECT tsp.*, t.uuid as ticket_uuid
      FROM ticket_supplies tsp
      JOIN tickets t ON tsp.ticket_id = t.id
      ORDER BY tsp.id
    `);
    const ticket_supplies = suppliesResult.rows;
    
    console.log(`Sending ${tickets.length} tickets, ${ticket_scans.length} scans, ${ticket_supplies.length} supplies`);
    ...
    try {
      ...
      res.json({
        ...
        sent: {
          ...
          supplies: ticket_supplies.length
        },
